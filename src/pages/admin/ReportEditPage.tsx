...
import { FileText, User, FolderOpen, MapPin, PlayCircle, ArrowLeft } from 'lucide-react';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Button } from '@/components/ui/button';
import { ReportsView } from '@/components/client/ReportsView';
...
const ReportEditPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    (async () => {
      // Busca relatório e project_id
      const { data, error } = await supabase.from('reports').select('*').eq('id', id).single();
      if (error) setError('Relatório não encontrado.');
      else {
        setReport(data);
  ...
  // Buscar todos os relatórios, roadmap, projeto, cliente, sprints para hierarquia
  useEffect(() => {
    if (!projectId) return;
    (async () => {
      const [{ data: reportsData }, { data: roadmapData }, { data: projectData }, { data: sprintsData }] = await Promise.all([
  ...
  if (!report) return <div className="p-8 text-gray-200 bg-gray-900 min-h-screen">Relatório não encontrado.</div>;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-gray-100 py-10">
      ...
      <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row gap-8">
        ...
        <aside className="hidden md:flex flex-col gap-4 w-56 p-4 bg-gray-800 rounded-xl border border-gray-700 shadow-lg min-h-[600px] text-sm">
          ...
          <div className="pl-4 border-l-2 border-red-700 ml-2">
            <label className="block text-xs font-medium mb-1 text-gray-400">Selecionar outro Relatório</label>
            <Select value={report.id} onValueChange={val => window.location.href = `/admin/reports/${val}/edit`}>
        ...
        <main className="flex-1 flex flex-col md:flex-row gap-8">
          <section className="flex-[1.5] bg-gray-900 rounded-xl border border-gray-700 shadow-lg p-12 min-w-[500px]">
            <h1 className="text-2xl font-bold mb-6 text-red-200 flex items-center gap-2"><FileText className="w-6 h-6" /> Editar Relatório</h1>
            <form className="space-y-5" onSubmit={e => { e.preventDefault(); handleSave(); }}>
              <div>
                <label className="block text-sm font-medium mb-1 text-gray-300">Título</label>
                <input name="title" value={report.title || ''} onChange={handleChange} className="w-full rounded bg-gray-800 border border-gray-700 px-3 py-2 text-gray-100" required />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1 text-gray-300">Conteúdo</label>
                <textarea name="content_md" value={report.content_md || ''} onChange={handleChange} className="w-full rounded bg-gray-800 border border-gray-700 px-3 py-2 text-gray-100 min-h-[120px]" required />
              </div>
              {error && <div className="text-red-400 text-sm">{error}</div>}
              <button type="submit" className="w-full py-2 rounded bg-red-700 hover:bg-red-800 text-white font-semibold transition" disabled={saving}>{saving ? 'Salvando...' : 'Salvar'}</button>
            </form>
          </section>
          {/* Preview do cliente: ReportsView filtrado */}
          <section className="flex-[2.2] bg-gray-950 rounded-xl border border-gray-800 shadow-lg p-12 flex flex-col items-center justify-center min-w-[700px]">
            ...
            {projectId && <ReportsView projectId={projectId} reportId={report.id} />}
          </section>
        </main>
      </div>
    </div>
